...
pub struct GitMergeParams {
    ...
    #[schemars(description = "Merge strategy: 'fast-forward' for fast-forward merge with commit, 'squash' for squash merge (default: fast-forward)")]
...
pub async fn git_merge(
    ...
    let strategy = params.strategy.unwrap_or_else(|| "fast-forward".to_string());
    ...
    if strategy == "squash" && params.commit_message.is_none() {
        return serde_json::json!(
            "Commit message is required for squash merge strategy."
        );
    }
    
    // Check if branch exists
    let mut check_branch_cmd = Command::new("git");
    check_branch_cmd.current_dir(&current_dir)
                   .arg("show-ref")
                   .arg("--verify")
    ...
    if strategy == "fast-forward" {
        ...
        squash_cmd.current_dir(&current_dir)
                 .arg("merge")
                 .arg("--squash")
                 .arg(&branch_name);

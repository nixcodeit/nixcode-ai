...
impl Config {
    ...
    pub fn get_model_for_provider(&self, provider: &str) -> String {
        ...
        match provider {
            "anthropic" => self.providers.anthropic.default_model.clone()
                .unwrap_or_else(|| DEFAULT_ANTHROPIC_MODEL.to_string()),
            "openai" => self.providers.openai.default_model.clone()
            ...
            "anthropic" => {
                ...
                if let Some(key) = &self.providers.anthropic.api_key {
                    ...
                    env::var("ANTHROPIC_API_KEY").map_err(|_| {
                        anyhow::anyhow!("ANTHROPIC_API_KEY environment variable not set and not configured")
                    })?
                }
            },
            "openai" => {
                ...
                if let Some(key) = &self.providers.openai.api_key {
                    ...
                    env::var("OPENAI_API_KEY").map_err(|_| {
                        anyhow::anyhow!("OPENAI_API_KEY environment variable not set and not configured")
                    })?
                }
            },
